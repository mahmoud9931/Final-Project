...
export class CartComponent implements OnInit {
  ...
  updateQuantity(item: any, newQuantity: number): void {
    console.log(`🔄 Updating ${item.product.name} quantity to ${newQuantity}`);
    ...
    this.http
      .delete(`http://localhost:3000/cart/${item.product._id}`, { headers })
      .subscribe({
        next: () => {
          const body = { product: item.product._id, quantity: newQuantity };
          this.http
  ...
  removeFromCart(productId: string): void {
    ...
    this.http
      .delete(`http://localhost:3000/cart/${productId}`, { headers })
      .subscribe({
        next: () => {
        ...
        error: (error) => {
          ...
          this.toastService.showError('Failed to remove product');
        },
      });
  }
